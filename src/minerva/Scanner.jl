function grn_scan(records::Dict{Int64,Any}; logger::Union{Nothing,SimpleLogger} = nothing)::VLResult
    ...
    try
        ...
        for (key, record) in records
            scan_result = grn_scan(record, synonym_dictionary; logger=logger)
...
function grn_scan(record::Array{String,1}, synonym_dictionary::Union{Nothing,Dict{String,String}} = nothing; 
    logger::Union{Nothing,SimpleLogger})::VLResult

    # initialize -
    canonical_record_array = Array{String,1}()
    canonical_token_dictionary = Array{MinervaToken,1}()
